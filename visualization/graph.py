...
def visualize_euler_path_with_arrows(G, euler_path, start_node, step=20):
	...
	gdf_line = gpd.GeoDataFrame(geometry=[LineString(coords)], crs="EPSG:4326").to_crs(epsg=3857)
	...
	arrow_gdf = gpd.GeoDataFrame(geometry=[LineString([a, b]) for a, b in arrow_lines], crs="EPSG:4326").to_crs(epsg=3857)
	...
	start_point.plot(ax=ax, color='yellow', markersize=80, zorder=4, label="Départ : Mairie")
